...
func TestCreateParamsIovec(t *testing.T) {
	tests := []struct {
		...
		name: "sysvshm-disable",
		config: CreateParams{
			Name:    "sysvshm",
			Root:    "/tmp/test/sysvshm/root",
			SysVShm: "disable",
		},
		iovec: []fakeIovec{{
			name: "name\x00",
			val:  []byte("sysvshm\x00"),
		}, {
			name: "path\x00",
			val:  []byte("/tmp/test/sysvshm/root\x00"),
		}, {
			name: "sysvshm\x00",
			val:  []byte{0, 0, 0, 0},
		}, {
		...
		name: "sysvshm-invalid",
		config: CreateParams{
			Name:    "sysvshm-invalid",
			SysVShm: "foobar",
		},
		err: errors.New(`jail: unknown SysVShm type "foobar"`),
	}, {
